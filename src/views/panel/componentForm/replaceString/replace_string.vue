            ...
            <el-table :data="node.params.fields"
              ...
              <el-table-column prop="outputField"
                ...
                <template slot-scope="scope">
                  <el-input v-if="scope.row.isEdit"
                            class="item"
                            v-model="scope.row.outputField"></el-input>
                  <div v-else
                       class="txt">{{scope.row.outputField}}</div>
                </template>
              </el-table-column>
              <el-table-column prop="searchStr"
                               label="搜索字符串"
                               align="center">
                <template slot-scope="scope">
                  <el-input v-if="scope.row.isEdit"
                            class="item"
                            v-model="scope.row.searchStr"></el-input>
                  <div v-else
                       class="txt">{{scope.row.searchStr}}</div>
                </template>
              </el-table-column>
              <el-table-column prop="replaceStr"
                               label="替换字符串"
                               align="center">
                <template slot-scope="scope">
                  <el-input v-if="scope.row.isEdit"
                            class="item"
                            v-model="scope.row.replaceStr"></el-input>
                  <div v-else
                       class="txt">{{scope.row.replaceStr}}</div>
                </template>
              </el-table-column>
