          ...
          <div>
            <el-table :data="node.params.fields"
              ...
              <el-table-column prop="filterType"
                               ...
                               align="center">
                <template slot-scope="scope">
                  <el-select v-model="scope.row.filterType" placeholder="请选择">
                    <el-option
  ...
  methods: {
    ...
    save () {
      ...
      this.data.nodeList.filter((node) => {
        if (node.id === this.node.id) {
          ...
          for (let i = 0; i < node.params.fields.length; i++) {
            node.params.fields[i].value = moment(node.params.fields[i].value).utcOffset(480).format('YYYY-MM-DD HH:mm:ss')
          }
          console.log(node.params.fields)
          this.$emit('save')
        }
      })
      this.visible = false
    ...
    handleDelete (row) {
      ...
      for (let i = 0; i < this.node.params.fields.length; i++) {
        if (this.node.params.fields[i].field === row.field && this.node.params.fields[i].maxValue === row.maxValue) {
